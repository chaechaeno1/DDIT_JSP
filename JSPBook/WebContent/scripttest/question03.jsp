	...
	$(function () {
		...
		$("td").on("click", function () {
			...
			if (dataArr.length == 0) {
				...
				// 같은 음료를 주문할 시 새로운 공간에 데이터를 추가하는게 아니라 같은 음료의 카운트를 증가시킨다.
				...
				if (dataFlag == false) {
					// 새로운 주문 시
					dataArr.push(orderMenu + "_" + orderPrice); // 새로 추가해주는 것
				}
			} // else 끝

			// 주문표를 dataArr 길이 만큼 다시 초기화해준다.
			for (var i = 0; i < dataArr.length; i++) {
				html += "<tr class='menu'>";
				...
				html += "	원</td>";
				html += "</tr>";
				price += parseInt(dataArr[i].split("_")[1]) * countArr[i]; // 수정된 부분
			}

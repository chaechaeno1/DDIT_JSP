						...
						<div class="table-responsive">
							...
							<div class="progress" style="height: 40px;">
								<div class="progress-bar progress-bar-striped" id="myBar" role="progressbar" aria-valuenow="60" aria-valuemin="0"
									...
									<span class="sr-only" id="span"></span>
								</div>
							</div>
							<br/>
							<h4 id="txt"></h4>
...
//progress-bar는 id가 myBar, width값을 변경하면됨, px이 아닌 %로 주기

//학생 랜덤 추첨기
   var myArray = ["김지호", "홍창용", "김선욱", "김영진", "이건정", "최룡", "강민택", "강진석", "장낙훈", "최성동", "한동욱", 
               "송시운", "김민채", "정소현", "최예원", "박상협", "전승표", "민지현", "전민균", "하지웅", "신찬섭", "서강민"];
...
function SeatMoveEvent(){   //출력 버튼 onclick 
   var elem = document.getElementById("myBar");//progress bar
   var width = 0;
   var seatMove = setInterval(move, 10);
   ...
   function move(){
      ...
      if(width >= 100){   // progressbar가 100%일 때
         ...
         var tds = document.getElementsByTagName("td"); //td태그 전부 가져오기
         ...
         clearInterval(seatMove); //100%일 때 프로그레스바 멈춤
      }else{   // 결과를 꾸미기 까지의 show
         seat = Math.floor(Math.random() * 36);   // 0~35 자리수
         ...
         if(width == 20){
            $("#txt").html("학생을 선정 중입니다!!!");
         }
         if(width == 30){
            $("#txt").html("학생을 출력 중입니다!!!");
         }if(width == 80){

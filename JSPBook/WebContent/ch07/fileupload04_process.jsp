                ...
                <div class="col-lg-12 col-md-12 col-12">
                    <div class="content-left wow fadeInLeft" data-wow-delay=".3s">
						...
						<%
							...
							//C드라이브의 upload라는 폴더가 없을 경우 오류가 발생하므로 검증필요
							//mkdir() : 가장 맨 끝 폴더를 설정(없다면 오류가 발생하므로.. mkdirs() 추천)
							//mkdirs() : 안에 있는 모든 경로 만들기
							if(!file.exists()){ // 설정한 경로에 폴더가 없으면
								file.mkdirs(); //폴더를 만들어준다.
							}
							
							//Commons-fileupload를 이용하여 파일을 업로드하려면 제일 먼저
							//Commons-fileupload 라이브러리가 필요
							//파일 업로드를 위해 패키지에 포함되어있는 DiskFileUpload 객체를 생성함.
							...
							//DiskFileUpload 객체 타입의 parseRequest() 메소드를 작성
							...
							int maxSize = 4 * 1024 * 1024;	//파일 업로드 시 1개의 파일당 사이즈(최대 사이즈)
							
							while(params.hasNext()){
								FileItem fileItem = (FileItem) params.next();
								
								if(fileItem.isFormField()){ //일반 데이터 일 때 
